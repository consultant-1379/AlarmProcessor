...
/**
 * Class handles alarms with record type NODE_SUSPENDED.
 */
public class NodeSuspendAlarmHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(NodeSuspendAlarmHandler.class);
    ...
    /**
     * Method handles alarms with record type NODE_SUSPENDED when there is a correlated alarm and when there is no correlated alarm in db.
    ...
    public AlarmProcessingResponse handleAlarm(final ProcessedAlarmEvent alarmRecord) {
        LOGGER.debug("Alarm received to NodeSuspendAlarmHandler : {}", alarmRecord);
        ...
        final Map<String, Boolean> response = currentServiceStateUpdator.updateForNodeSuspendedAlarm(alarmRecord);
        ...
        // setting InitiateAlarmSync attribute in alarmProcessingResponse to trues
